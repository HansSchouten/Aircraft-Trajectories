namespace AircraftTrajectories.Models
{
    ...
    public class ColorMap
    {
        ...
        public int[,,] Custom(MainWindowViewModel data)
        {
            int maxX = data.RawData.Length;
            int maxY = data.RawData[0].Length;
            double min = 255;
            double max = 0;
            for (int x = 0; x < maxX; x++)
            {
                ...
                {
                    if (data.RawData[x][y] < min) {
                        min = data.RawData[x][y];
                    }
                    if (data.RawData[x][y] > max) {
                        max = data.RawData[x][y];
                    }
                }
            }
            ...
            for (int i = 0; i < maxX; i++)
            {
                ...
                {
                    double factor = (data.RawData[i][p] - min) / (max - min);
